...
/**
 *
 * Finds the closest road location to the original location
 *
 * @param    int $latitude The latitude of the position
 * @param    int $longitude The longitude of the position
 * @return   array Array containing the latitude,longitude of the location
 *
 */
function getRoadLocation($latitude, $longitude) {
	//Get latitude and logitude for the street
	$url = "http://maps.googleapis.com/maps/api/directions/json?origin=$latitude,$longitude&destination=$latitude,$longitude&sensor=false&mode=walking";
	$data = file_get_contents($url);
	$jsondata = json_decode($data,true);
	$status = $jsondata ['status'];
	if ($status == 'ZERO_RESULTS') {
		//The location is invalid, possibly in the sea? Return original predicted location
		$lat = $latitude;
		$lng = $longitude;
	} else {
		$lat = $jsondata ['routes'][0]['legs'][0]['start_location']['lat'];
		$lng = $jsondata ['routes'][0]['legs'][0]['start_location']['lng'];
	}
	#echo "Predicted Location (Road): $lat $lng<br><br>";
...
function distance($lat1, $lon1, $lat2, $lon2) {
	$url = "http://maps.googleapis.com/maps/api/distancematrix/json?origins=$lat1,$lon1&destinations=$lat2,$lon2&mode=walking&sensor=false";
	$data = file_get_contents($url);
	$jsondata = json_decode($data,true);
	$dist = $jsondata ['rows'][0]['elements'][0]['distance']['value'];
	/*$theta = $lon1 - $lon2;
	...
	$km = $dist/1000;
	$miles_val= "0.621";
	$miles = intval(intval($km) * $miles_val);  
	#echo "Distance between $lat1,$lon1 and $lat2,$lon2 using roads: $km km<br>";
	return $km;
}
...
function predictMain($locations) {
	...
	#echo "Predicted location: $predictLat,$predictLon <br>";
	$locations[] = getRoadLocation($predictLat,$predictLon);
	...
	$type0 = $jsondata ['results'][0]['address_components'][0]['types'][0];
	$type1 = $jsondata ['results'][0]['address_components'][1]['types'][0];
	$type2 = $jsondata ['results'][0]['address_components'][2]['types'][0];
	if ($type0 == 'route') {
		$place = $jsondata ['results'][0]['address_components'][0]['long_name'];
	} else if ($type1 == 'route') {
		$place = $jsondata ['results'][0]['address_components'][1]['long_name'];
	} else if ($type2 == 'route') {
		$place = $jsondata ['results'][0]['address_components'][2]['long_name'];
	} else {
		$place = $jsondata ['results'][0]['address_components'][1]['long_name'];
	}
